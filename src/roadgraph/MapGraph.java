...
public class MapGraph {
     ...
     * Get the number of vertices (road intersections) in the graph
     ф     * @return The number of vertices in the graph.
     */
    ...
    public List<GeographicPoint> dijkstra(GeographicPoint start, GeographicPoint goal, Consumer<GeographicPoint> nodeSearched) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (!visited.contains(currentNode)) {
                ...
                for (MapEdge edgeBetveen : currentNode.getEdges()) {
                    MapNode neighbor = edgeBetveen.getEndNode();
                    if (!visited.contains(neighbor)) {
                        double distBetween = currentNode.getDistance() + edgeBetveen.getLength();
                        if (distBetween < neighbor.getDistance()) {
                            neighbor.setDistance(distBetween);
    ...
    public List<GeographicPoint> aStarSearch(GeographicPoint start,
        ...
        while (!toExplore.isEmpty()) {
            ...
            if (!visited.contains(currentNode)) {
                ...
                if (currentNode.equals(endNode)) break;
                for (MapEdge edgeBetveen : currentNode.getEdges()) {
                    MapNode neighbor = edgeBetveen.getOtherNode(currentNode);
                    if (!visited.contains(neighbor)) {
                        //double weight = neighbor.getDistance();
                        double distThrouNext = currentNode.getDistance() + edgeBetveen.getLength();
                        neighbor.setDistance(distThrouNext);
                        if (distThrouNext <= neighbor.getDistance()) {
                            // neighbor.setDistance(distThrouNext);
                            parentMap.put(neighbor, currentNode);
                            toExplore.add(neighbor);
                        }
